...
{
    CONSOLE_LOG_INFO << "client connect cb: " << conn->get_name();
}

void Client_ClosedCB(const zysoft::net::tcp_connection_ptr& conn)
{
    CONSOLE_LOG_INFO << "client closedcb: " << conn->get_name();
}

void Client_MessageCB(const zysoft::net::tcp_connection_ptr& conn, zysoft::net::buffer* buffer)
{
    CONSOLE_LOG_INFO << "client messagecb: " << conn->get_name() << " " << buffer->readable_bytes();
    ...
    CONSOLE_LOG_INFO << conn->get_name() << " received bytes: " << (s.size() + 2) << " content: " << s;
}

void Client_WriteCB(const zysoft::net::tcp_connection_ptr& conn, std::size_t len)
{
    CONSOLE_LOG_INFO << "client writecb: " << conn->get_name() << " send bytes: " << len;
}
...
void Test1()
{
    ...
    pool.Start();
    // 普通ping测试
    const std::string host = "192.168.1.28";
...
int main()
{
    try {
        ...
        CONSOLE_LOG_CRIT << "exception: " << e.what();
    }
    return 0;
}
