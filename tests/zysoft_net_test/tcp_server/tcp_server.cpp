...
{
    CONSOLE_LOG_INFO << "ConnectionCB new conn: " << conn->get_name();
}

void Server_TimeoutCB(const net::tcp_connection_ptr& conn, net::timeout_type type, std::uint64_t duration)
{
    CONSOLE_LOG_INFO << "ConnectionCB timeout cb conn: " << conn->get_name() << " timeout type: "
        << net::util::to_string(type) << " sec: " << duration;
}

void Server_CloseCB(const net::tcp_connection_ptr& conn)
{
    CONSOLE_LOG_INFO << "ConnectionCB close cb conn: " << conn->get_name();
}

void Server_ErrorCB(const net::tcp_connection_ptr& conn, const net::error_code& ec)
{
    CONSOLE_LOG_INFO << "ConnectionCB error cb conn: " << conn->get_name() << " error code: " << ec.value() << " reason: " << ec.message();
}

void Server_WriteCB(const net::tcp_connection_ptr& conn, std::size_t len)
{
    CONSOLE_LOG_INFO << "ConnectionCB write cb conn: " << conn->get_name() << " len: " << len;
}
...
void Server_ReadCB(net::tcp_connection_ptr conn, net::buffer* buffer)
{
    ...
    CONSOLE_LOG_INFO << "MessageCB size: " << buffer->readable_bytes();
    //buffer->Clear();
    ...
    conn->send(s2 + "<<\r\n");
    if (s2 == "shutdown") {
        CONSOLE_LOG_INFO << "------------------------> shutdown " << conn->get_name();
        for (int i = 0; i != 10; ++i) {
    ...
    CONSOLE_LOG_INFO << "MessageCB " << s2 << " size: " << buffer->readable_bytes();
}
...
void Server_ReadCB_Echo(net::tcp_connection_ptr conn, net::buffer* buffer)
{
    CONSOLE_LOG_INFO << "MessageCB size: " << buffer->readable_bytes();
    if (1) {
        const char* p = buffer->readable_ptr();
        std::string sx(p, p + buffer->readable_bytes());
        CONSOLE_LOG_INFO << "MessageCB size: " << sx;
    }
    if (1) {
        const char* p = net::util::find_CRLF(*buffer);
        if (!p) {
...
void TestEcho()
{
    try {
        ...
        CONSOLE_LOG_CRIT << "TestEcho exception: " << e.what();
    }
}
