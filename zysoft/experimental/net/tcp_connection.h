...
private:
    ...
    void async_write()
    {
        if (write_timeout_sec_ > 0 && !write_timer_) {
    ...
    void handle_read(boost::system::error_code ec, std::size_t len)
    {
        ...
        NET_LOG_TRACE << get_name() << " read bytes: " << len;
    ...
    void handle_read_write_error(const boost::system::error_code& ec)
    {
        // 此函数有2种情况会被调用:
    ...
    void handle_timeout(timeout_type type, std::uint64_t sec)
    {
        assert_run_in_loop();
        if (state_ == EState::Closed) {
            NET_LOG_WARN << "conn: " << get_name() << " timeout: " << static_cast<int>(type) << ". but state is Closed";
            return;
        }
        ...
        if (type == timeout_type::read) {
            assert(read_timer_);
            read_timer_ = nullptr;
            NET_LOG_TRACE << "read timeout " << get_name() << " sec: " << sec;
        } else {
            assert(write_timer_);
    ...
    void reset_timer(timer_queue::timer_handle_ptr& ptr)
    {
        if (ptr)
            get_event_loop()->get_timer_queue()->reset_timer(ptr);
    }
...
private:
    ...
    std::uint64_t                   read_timeout_sec_;
    std::uint64_t                   write_timeout_sec_;
};
