...
public:
    ...
    struct entry
    {
        ...
        entry(std::uint64_t tick, std::uint64_t d, const T& v)
            : tick_(tick), duration_(d), value_(v)
        {
        }
        std::uint64_t tick_;
    ...
    using wheel_handle = std::weak_ptr<entry>;
...
public:
    ...
    wheel_handle add(const T& v, duration_type duration)
    {
        return add_impl(duration, v);
    }

    wheel_handle add(T&& v, duration_type duration)
    {
        return add_impl(duration, std::move(v));
    }

    void cancel(wheel_handle hdl)
    {
        ...
        if (it != all_entries_.end()) {
            entry_ptr e = *it;
            slot* s = get_slot(e);
            slot_remove_entry(s, e);
            all_entries_.erase(it);
        }
        assert(all_entries_.size() == wheel_entries_count() + immediate_entries_.size());
    }

    void reset(wheel_handle hdl)
    {
        entry_ptr p = hdl.lock();
        if (!p || p->duration_ == 0)
            return;
...
private:
    ...
    wheel_handle add_impl(duration_type duration, TT v)
    {
        assert(all_entries_.size() == wheel_entries_count() + immediate_entries_.size());
        if (duration == 0) {
            auto e = std::make_shared<entry>(current_tick, duration, std::forward<TT>(v));
