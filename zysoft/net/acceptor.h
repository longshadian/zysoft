#ifndef _ZYSOFT_NET_ACCEPTOR_H
#define _ZYSOFT_NET_ACCEPTOR_H
...
public:
    ...
    bool listen(std::string* err_msg = nullptr)
    {
        try {
            ...
            acceptor_->bind(ep);
            acceptor_->listen();
            ZYSOFT_NET_TRACE("init acceptor success listening: %s",  addr_.ip_port_to_string().c_str());
            return true;
