#ifndef _ZYSOFT_NET_TCP_CLIENT_H
#define _ZYSOFT_NET_TCP_CLIENT_H

#include <memory>

#include <zysoft/net/types.h>
#include <zysoft/net/connector.h>
...
public:
    tcp_client(event_loop* loop, const inet_address& addr)
        : loop_(loop)
        , addr_(addr)
        //, connection_cb_(std::bind(&default_close_cb, std::placeholders::_1))
        //, message_cb_(std::bind(&default_message_cb, std::placeholders::_1, std::placeholders::_2))
    ...
    {
        conn_->get_connection()->set_connection_callback(std::bind(&default_close_cb, std::placeholders::_1));
        conn_->get_connection()->set_read_callback(std::bind(&default_message_cb, std::placeholders::_1, std::placeholders::_2));
        conn_->get_connection()->set_write_complete_callback(std::bind(&default_write_complete_cb, std::placeholders::_1, std::placeholders::_2));
    }
...
private:
    ...
    void remove_connection(const tcp_connection_ptr& conn)
    {
        ...
        ZYSOFT_NET_TRACE("tcp client remove connection: %s", name.c_str());
...
#endif // !_ZYSOFT_NET_TCP_CLIENT_H
